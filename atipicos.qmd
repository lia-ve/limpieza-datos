...
> Los valores atípicos (outliers) son observaciones que se desvían significativamente de otras observaciones en un conjunto de datos. Estas discrepancias pueden ser el resultado de errores en la medición, variaciones naturales de la muestra, factores externos inusuales, entre otros, dependiendo en gran medida del objeto de estudio.
...
-   **Multivariados**: Valores atípicos que dependen de la relación entre dos o más variables. Por ejemplo, un nivel de CO2 que resulta inusualmente alto cuando es visto en contexto con la temperatura y la humedad.
...
Un cuartil es una división del conjunto de datos que corta en tres puntos distintos a lo largo del conjunto (ordenado ya de menor a mayor, o según la frecuencia de cada valor) para terminar con cuatro partes de casi igual tamaño. Estos tres puntos son los cuartiles, y se obtienen a partir de tres medianas: la primera, para el conjunto de datos entero, cortandolo en o cerca de la mitad, y la segunda y tercera, a partir de las dos mitades resultantes.

![Ejemplo de calculo de rango intercuartílico (sacado de geeksforgeeks.org)](images/Interquartile-Range-Calculation.png)

Después de sacar los tres cuartiles, se suele multiplicar el cuartil $Q_1$ y el cuartil $Q_2$ por un factor para buscar valores atípicos en los extremos del conjunto.
...
Este método asume que los datos siguen una distribución normal y utiliza la media y la desviación estándar para detectar outliers. Ambos se pueden calcular mediante pandas sin mayor problema.
...
-   **Boxplots**: Muestran la distribución de datos y resaltan valores atípicos. Generalmente se generan a partir del rango intercuartílico descrito anteriormente.
